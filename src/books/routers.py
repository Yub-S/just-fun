...
async def create_a_book(bookdata:createbook,session:AsyncSession=Depends(get_session))->dict:
    new_book = await book_service.create_a_book(bookdata,session)
    return new_book

@book_router.get("/{book_uid}",response_model=book)
async def get_a_single_book(book_uid:str,session:AsyncSession=Depends(get_session))->dict:
...
async def update_record_of_a_book(book_uid:str, newdetails:updatebook,session:AsyncSession=Depends(get_session))-> dict:
...
async def remove_a_book(book_uid:str,session:AsyncSession=Depends(get_session)):
